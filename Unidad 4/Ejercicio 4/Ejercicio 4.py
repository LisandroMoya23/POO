...
class Calculadora(object):
    ...
    __primerOperandoIm=None
    __segundoOperandoIm=None
    def __init__(self):
        self.__ventana = Tk()
        ...
        ttk.Button(mainframe, text='C', command=partial(self.borrarPanel)).grid(column=2, row=8, sticky=W)
        self.__panel.set('0')
        panelEntry.focus()
    ...
    def ponerNUMERO(self, numero):   
        ...
        elif self.__operadorAux=='i':
            if self.__primerOperandoIm==None:
                valor = self.__panel.get()
                self.__panel.set(numero)
                if valor!='0':
                    self.__primerOperandoIm=Imaginario(int(valor),int(numero))
                    self.__panel.set(self.__primerOperandoIm)
            else:
                valor = self.__panel.get()
                self.__panel.set(numero)
                if valor!='0':
                    self.__segundoOperandoIm=Imaginario(int(valor),int(numero))
                    self.__panel.set(self.__segundoOperandoIm)
        else:
            self.__operadorAux=None
            valor=self.__panel.get()
    ...
    def borrarPanel(self):
        ...
        self.__primerOperandoIm=None
        self.__segundoOperandoIm=None
        self.__operadorAux=None
    ...
    def ponerOPERADOR(self, op):
        if op=='=':
            if self.__operadorAux=='i':
                operacion=self.__operador.get()
                self.resolverOperacion(self.__primerOperandoIm, operacion, self.__segundoOperandoIm)
                self.__operador.set('i')
                self.__segundoOperandoIm=None
            else:
                operacion=self.__operador.get()
