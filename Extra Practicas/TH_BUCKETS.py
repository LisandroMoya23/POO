...
class TablaHash:
    ...
    def buscar1(self,valor):
        d = self.metodoExtraccionClaves(valor,self.__dimension)
        j=0
        while j< self.__dimensionBucket and self.__tabla[d][j]!= valor:
            j+=1
        if j < self.__dimensionBucket:
            print("Se encontro")
        else:
            d = self.metodoExtraccionClaves(valor,self.__dimensionOverflow)
            j=0
            while j<self.__dimensionBucket and self.__tablaOverflow[d][j] != valor:
                j+=1
            if j < self.__dimensionBucket:
                print("Se encontro en la tabla de overflow")
            else:
                print("Error no se ecnontro")
    def mostrar(self):
        for i in range(0,self.__dimension):
